...
puts("Data Read-in took "+elapsed.to_s+" seconds")
...
#print('S')
#print(S)
def optimize(bldgType, vintageID, climateZone, objective, budget, paybackYears, top_N)
	...
	prev_results = [] # Additional row of A matrix for dynamically added constraints
	# Iterate to get top_N best solutions
	(0..top_N-1).each do |iter|
		if prev_results.length() > 0 #only generate new rows when there are results
			new_rows = p.add_rows(1)
			# Get the sum of results[i]
			sum = 0
			(0..prev_results.length()-1).each do |i|
				sum = sum + prev_results[i]
			end
			new_rows[0].name = 'new_constraint_'+(iter).to_s
			new_rows[0].set_bounds(Rglpk::GLP_UP, nil, sum-1)
			
			puts('NUMBER ROWS:')
			puts(p.rows.size())
		end
		
		a_matrix_flatten = a_matrix_flatten + prev_results # Initially prev_results is empty
		puts('A matrix size')
...
puts('Top N best solution:')
top_N = gets.chomp.to_i
# Run optimization:
optimize(bldgType, vintageID, climateZone, objective, budget, paybackYears, top_N)
